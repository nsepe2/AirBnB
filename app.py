    ...
    # Seller Page
        ...
        if st.button("Predict Review Score"):
            ...
            try:
                ...
                # Make prediction
                st.session_state['predicted_score'] = model.predict(input_data_scaled)[0]

                #Check if Predicted score is greater than 5
                if st.session_state['predicted_score'] > 5:
                    ...
                    st.markdown(f"<h2 style='font-size: 36px; color: #FF5733; font-weight: bold;'>The predicted review score for your listing is: {st.session_state['predicted_score']:.2f}</h2>", 
                                unsafe_allow_html=True)
            except ValueError as e:
